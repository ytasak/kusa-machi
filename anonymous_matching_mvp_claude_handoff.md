...
ただし所持上限が 12 なので、17 を同時に持つことはできない。満額まで回復を
受け取るには、使いながら回復する必要がある。Match の成立にも Like の消費が
必要で、時間回復には時間の経過が必要なので、17 に届くのは相当に使い込んだ
日だけになる。
...
起点は回復を1つ付与するたびに3時間ぶん進む。ちょうど3時間の境目に見に来なくても
余った経過時間は残るので、回復が積み残ることはない。2つ目以降の Like で起点が
今に進むことはない。使うほど回復が遠のいてしまうため。

所持上限に達している状態で3時間が来た場合は、**何も増えず、回復回数も消費しない**。
起点も動かない。よって Like を使って空きができた時点で、取り置かれていた
経過時間がそのまま回復に変わる。1日2回という天井は変わらない。
...
> 回復を補助として足した。
>
> 希少性を壊さないよう、ここでも天井は3つ重ねている: 3時間という間隔、1日2回
> という回数、そして所持上限 12。ソシャゲ的なスタミナシステムには発展させない。
>
...
回復して 11 や 12 になったときに「11 / 10」と表示されてしまう。
...
time_recovery_count SMALLINT NOT NULL DEFAULT 0,
...
時刻が入り、回復を1つ付与するごとに3時間進む。`NULL` は「まだ Like を使って
...
- time_recovery_count は 0 以上 2 以下
...
画面が軽い通知を出す。残数を返す他のエンドポイントも同じ2項目を返す。
...
カードのほかに `remaining_likes` / `next_recovery_at` / `likes_recovered` も
返す。探す画面は長く開かれたままになるので、カードを継ぎ足すこの往復が
時間回復を画面へ反映する機会にもなる（§8 を参照）。
...
- 何時間経っても1日2回を超えない
- 所持上限 12 を超えない
- 上限で回復できなかった場合は回復回数も起点も動かない。空きができたら
  取り置かれていた経過時間がそのまま回復に変わる
