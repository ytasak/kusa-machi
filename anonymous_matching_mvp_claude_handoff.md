...
ただし所持上限が 12 なので、15 を同時に持つことはできない。満額まで回復を
受け取るには、使いながら回復する必要がある。
...
Like の回復にあわせて、当日の報酬の状態も後から追加した（§8 を参照）:

```sql
bonus_likes SMALLINT NOT NULL DEFAULT 0,
profile_reward_claimed BOOLEAN NOT NULL DEFAULT FALSE,
match_reward_count SMALLINT NOT NULL DEFAULT 0
```

残数は保存せず `10 + bonus_likes - 送信済み Like 数` で導出する。所持上限は
付与の時点で切り詰めるので、`bonus_likes` には実際に増えた分だけが入る。
...
- annual_income >= 0
- exposure_count >= 0
- bonus_likes は 0 以上 5 以下（プロフィール +1 と Match +2 × 2 の合計）
...
3. 自分自身なら拒否する
4. 重複なら拒否する
5. 残数 > 0 を強制する（予算そのものではなく、回復ぶんを含めた残数で見る）
6. Like を INSERT する
